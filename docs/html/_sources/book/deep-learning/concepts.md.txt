...
- **Feed Forward:** takes an input, passes it through multiple layers of hidden
  neurons and outputs a prediction representing the combined input of all the
  neurons.
- **Input Layer:** is the layer consisting of the input data which is passed to
  the first hidden layer in the network. Each layer in the network consists of
  a series of neurons. 
- `Neuron` is the weighted sum of the inputs plus bias and then feeds the sum
...
- **Hidden Layers:** reside in-between input and output layers and can consist
  of ‘n’ layers which can  be a hyperparameter to tune for. 
...
- We perform this backward pass to adjust the weights and biases in the network
...
- In SGD, we repeatedly run through the training set data and update the
  parameters according to the  gradient of the error with respect to the
...
- In SGD fluctuation enables it to jump to new and potentially better local
...
- In a neural network, the raw predictions which come out of the last layer of
  the network - which converts these values to probabilities for multi-class
  labels. 
